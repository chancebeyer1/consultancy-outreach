...
When someone runs a tool and gives an email, we email them a copy of their result via Resend — a
touchpoint + a record they keep + proof our email lands. Wrapped so a failure NEVER affects the
on-page result the user already has, and gated on a verified Resend sending domain (NEWSLETTER_FROM);
until that's set up it no-ops cleanly, so shipping this is safe before the domain is verified.
"""
...
def deliver(email: str | None, name: str | None, kind: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Email the result. `kind` is 'audit' or 'roast'. Returns what happened."""
    out = {"emailed": False}
    if not email:
        return out

    # Email them their result. Gated on a verified Resend domain; no-ops gracefully otherwise.
    try:
        subject, body = _format(kind, payload)
        from clients import resend
        from config import Config

        resend.send(to_email=email, subject=subject, text=body, from_addr=Config.transactional_from)
        out["emailed"] = True
...
def _format(kind: str, p: dict[str, Any]) -> tuple[str, str]:
    ...
    if kind == "audit":
        ...
        body = (
            ...
            "— Chance, Agentry\n\n"
            "(You're getting this because you ran our free audit.)"
        )
        return subject, body
    ...
    body = (
        ...
        "— Chance, Agentry\n\n"
        "(You're getting this because you ran our free roaster.)"
    )
    return subject, body
